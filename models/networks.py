...
class SrResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        Parameters:
            input_modal(int)    -- the number of channels in input modal
            input_nc (int)      -- the number of channels in input images
            output_nc (int)     -- the number of channels in output images
            ngf (int)           -- the number of filters in the last conv layer
        ...
        model = [nn.ReflectionPad2d(3),
                 ...
                 norm_layer(ngf),
                 nn.ReLU(True)]
        self.add_module('encoder_0', nn.Sequential(*model))
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model = [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                     norm_layer(ngf * mult * 2),
                     nn.ReLU(True)]
            self.add_module('encoder_{}'.format(i + 1), nn.Sequential(*model))
    ...
    def forward(self, x):
        ...
        for i in range(self.n_downsampling + 1):
            x = self.__getattr__('encoder_{}'.format(i))(x)
        out = self.decoder(x)
        return out
...
class MultiModalAttetionLayer(nn.Module):
    def __init__(self, modal_num=3, input_size=64, reduction=16):
        super(MultiModalAttetionLayer, self).__init__()
...
class MultiHeadResnetGenerator(nn.Module):
    ...
    def __init__(self, attention_type, n_input_modal, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d,
        ...
        """
        assert (n_blocks >= 0)
        self.attention_type = attention_type  # ['multi_modal_attention', 'single_modal_attention']
        if 'single_modal_attention' in self.attention_type:
            print('using single modal attention')
        if 'multi_modal_attention' in self.attention_type:
            print('using multi modal attention')
        ...
        for encoder_idx in range(n_input_modal):
            ...
            for i in range(n_downsampling):  # add downsampling layers
                ...
                if 'single_modal_attention' in self.attention_type:
                    model.append(SingleModalAttetionLayer(ngf * mult * 2))
                self.add_module('encoder_{}_{}'.format(encoder_idx, i + 1), nn.Sequential(*model))

        if 'multi_modal_attention' in self.attention_type:
            self.multi_modal_attention = MultiModalAttetionLayer(n_input_modal)
    ...
    def forward(self, input, train=False):
        ...
        if 'multi_modal_attention' in self.attention_type:
            features = self.multi_modal_attention(modal_features)
        else:
        ...
        if train:
            return out, encoder_features, fused_features
        else:
            return out
